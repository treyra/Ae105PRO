...
def main():
    ...
    #Fit the orbital paramters using non-linear least squares regression (seems to get stuck in local minima
    ...
    for i in range(3):
        ...
        vNumerical[:,i] = np.gradient(vNumerical[:,i], .001)
        aNumerical[:,i] = np.gradient(xSpline[:,i], .001)
        
    

    #Compute position and velocity errors as a function of time
    xFitError = xFit - xTruth
    ...
    plt.plot(tTruth,np.linalg.norm(xSplineError[:,3:],axis=1),label="Spline")
    ...
    plt.plot(tTruth,np.linalg.norm(vSplineError,axis=1),label="Spline")
    ...
    plt.plot(tTruth,np.linalg.norm(vNumericalError,axis=1),label="Spline")
    ...
    plt.plot(tTruth,np.linalg.norm(vNumericalError,axis=1),label="Spline on Numerical Derivative of Spline")
    plt.plot(tTruth,np.linalg.norm(vSplineError,axis=1),label="Spline on Numerical Derivative")
    plt.plot(tTruth,np.linalg.norm(xSplineError[:,3:],axis=1),label="Spline")
    ...
    plt.plot(tTruth,np.linalg.norm(xFitError[:,:3],axis=1),label="Least Squares")
    plt.plot(tTruth,np.linalg.norm(xSplineError[:,:3],axis=1),label="Spline")
    ...
    #Compare using true paramaters, to show J2 effects lead to greater errors than spline interpolation
    xFit = OrbitInterpolation.keplerainOrbitModel(paramsTruth,tTruth)
    xFitError = xFit - xTruth

    #Plot and compare
    plt.figure()

    plt.plot(tTruth,np.linalg.norm(xFitError[:,:3],axis=1),label="True Keplarian Parameters")
    plt.plot(tTruth,np.linalg.norm(xSplineError[:,:3],axis=1),label="Spline")
    

    plt.xlabel("Time (s)")
    plt.ylabel("Position Error (km)")
    plt.title("Errors between keplarian model, spline fit vs ground truth data")
    plt.legend()
    plt.draw()
